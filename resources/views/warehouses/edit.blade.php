...
<div class="max-w-3xl">
    <div
        ...
        <div class="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <h3 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Node Optimization
            </h3>
            <p class="text-sm text-slate-500">Update the physical location or identity of this warehouse node.</p>
        </div>

        <form action="{{ route('dashboard.warehouses.update', $warehouse) }}" method="POST" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="location_code" id="location_code" required
                        value="{{ old('location_code', $warehouse->location_code) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <input type="text" name="address[street]" placeholder="Street Address"
                            value="{{ old('address.street', $warehouse->address['street'] ?? '') }}"
                            class="w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-4 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all dark:text-white">
                    </div>
                    ...
                    <input type="text" name="address[state]" placeholder="State / Province"
                        value="{{ old('address.state', $warehouse->address['state'] ?? '') }}"
                        class="w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-4 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all dark:text-white">
                    <input type="text" name="address[country]" placeholder="Country"
                        value="{{ old('address.country', $warehouse->address['country'] ?? '') }}"
                        class="w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-4 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all dark:text-white">
